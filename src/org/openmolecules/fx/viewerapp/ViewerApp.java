...
import org.openmolecules.pdb.PDBFileParser;
...
public class ViewerApp extends Application {
	...
	private static final String MICROSCOPE_DATA_PATH = "/home/thomas/data/3D-microscope/";
	public static final String VOXEL_DATA_BONSAI = VOLVIS_DATA_PATH + "bonsai_256x256x256.raw";
	public static final String VOXEL_DATA_SKULL = VOLVIS_DATA_PATH + "skull_256x256x256.raw";
	public static final String VOXEL_DATA_LOBSTER = VOLVIS_DATA_PATH + "lobster_301x324x56.raw";
	public static final String VOXEL_DATA_3D_MICROSCOPE = MICROSCOPE_DATA_PATH + "normal fibroblast_3.vol";
	public static final String VOXEL_DATA_FILE = null;  // use one of above or use null for molecules

	public static final boolean VERBOSE_POINTS_AND_TRIANGLES = false;    // print addPoint and addTriangle

	private static final boolean SHOW_TEST_MOLECULES = true;
	private static final int TEST_MOLECULE_MIN = 0;	// between 0 and TEST_MOLECULE_MAX
	private static final int TEST_MOLECULE_MAX = 63;	// 12 or less

	private static final boolean TEST_PROTEIN = false;
	private static final boolean TEST_CONFORMERS = false;
	private static final boolean TEST_MOLECULE_SURFACES = true;
...
//	private static final String TEST_MOL_FOR_CONFORMERS = "e`TZFH@NAEGFiJemgo`@cIEEEELjeLeMBhhbiPHhXdUSUSSPACUMSTHHSi@@";
	...
	private void buildMolecules(V3DScene scene) {
		if (TEST_PROTEIN) {
			PDBFileParser parser = new PDBFileParser();
			try {
System.out.print("loading file...  ");
//				Molecule3D mol = parser.load("/home/thomas/data/pdb/3ow2.pdb");	// very large
//				Molecule3D mol = parser.load("/home/thomas/data/pdb/1M19.pdb");	// large
				Molecule3D mol = parser.load("/home/thomas/data/pdb/reninOpenFlap.pdb"); // medium
//				Molecule3D mol = parser.load("/home/thomas/data/pdb/1crn.pdb");	// small
System.out.println("done");
				Conformer conformer = new Conformer(mol).center();
				V3DMolecule vm = new V3DMolecule(conformer, MoleculeArchitect.CONSTRUCTION_MODE_WIRES);
//				vm.setSurfaceMode(0, (2 + i) % 3, SurfaceMesh.SURFACE_COLOR_ATOMIC_NOS);
...
//			ConformerGenerator cg = new ConformerGenerator(1);
			...
			while (true) {
				...
				if (count == 0) {
					...
					vm.setTranslateX(refCoord.x - coord.x);
					vm.setTranslateY(refCoord.y - coord.y);
					vm.setTranslateZ(refCoord.z - coord.z);
				}
				scene.addMolecule(vm);
				hue += 37;
				count ++;
		...
		else if (TEST_MOLECULE_SURFACES) {
			int count = 2;
			for (int i = 0; i < count*count; i++) {
				...
				vm.setTranslateX(8 * (i/count - shift));
				vm.setTranslateY(8 * (i%count - shift));
//				vm.setTranslateZ(8 * coord[2]);
//				vm.rotate(new Rotate(225, new Point3D(1.0, 0, 0)));
				scene.addMolecule(vm);
			}
		}
		else if (SHOW_TEST_MOLECULES) {
...
//				vm.setSurfaceTransparency(0, 0.2 * (i % 5));
				...
				vm.activateEvents();
				vm.setTranslateX(POSITION_FACTOR * coord[0]);
				vm.setTranslateY(POSITION_FACTOR * coord[1]);
				vm.setTranslateZ(POSITION_FACTOR * coord[2]);
//				vm.rotate(new Rotate(225, new Point3D(1.0, 0, 0)));
				scene.addMolecule(vm);
			}
		}
	...
	public void start(Stage primaryStage) {
		V3DSceneWithSidePane viewer = new V3DSceneWithSidePane();
		String css = getClass().getResource("/resources/molviewer.css").toExternalForm();
		Scene scene = new Scene(viewer, 1024, 768, true, SceneAntialiasing.BALANCED);
		scene.getStylesheets().add(css);
		viewer.getScene3D().widthProperty().bind(scene.widthProperty());
		viewer.getScene3D().heightProperty().bind(scene.heightProperty());

		buildMolecules(viewer.getScene3D());
