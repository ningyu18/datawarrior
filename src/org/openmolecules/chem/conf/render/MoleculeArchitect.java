...
public class MoleculeArchitect {
	...
	public static final int DEFAULT_CONSTRUCTION_MODE = CONSTRUCTION_MODE_BALL_AND_STICKS;
	public static final int DEFAULT_HYDROGEN_MODE = HYDROGEN_MODE_ALL;
	...
	public MoleculeArchitect(MoleculeBuilder builder) {
		...
		mConstructionMode = DEFAULT_CONSTRUCTION_MODE;
...
/*	public void centerMolecule(Conformer conformer) {
	...
	public void buildMolecule(Conformer conformer) {
		...
		if (mConstructionMode != CONSTRUCTION_MODE_BALLS)
		...
		if (mConstructionMode != CONSTRUCTION_MODE_WIRES) {
			for (int atom=0; atom<mol.getAllAtoms(); atom++) {
				if (!skipAtom(mol, atom)) {
					...
					double radius = (mConstructionMode == CONSTRUCTION_MODE_STICKS) ? STICK_SBOND_RADIUS
					...
					mBuilder.addSphere(atom, -1, conformer.getCoordinates(atom), radius, ATOM_ARGB[atomicNo]);
					}
				}
			}
	...
	private void buildStickBond(Conformer conformer, int bond, double d, double b, double c) {
		...
		int color1 = ATOM_ARGB[mol.getAtomicNo(mol.getBondAtom(0, bond))];
		int color2 = ATOM_ARGB[mol.getAtomicNo(mol.getBondAtom(1, bond))];
