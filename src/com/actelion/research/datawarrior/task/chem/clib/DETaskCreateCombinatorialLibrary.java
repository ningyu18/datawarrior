...
public class DETaskCreateCombinatorialLibrary extends AbstractTask implements Runnable,TaskConstantsCLib {
	...
	public DETaskCreateCombinatorialLibrary(DEFrame owner, DataWarrior application) {
		super(owner, true);
	...
	public boolean isConfigurationValid(Properties configuration, boolean isLive) {
		...
		for (int i = 0; i < reactantCount; i++)
			if (configuration.getProperty(PROPERTY_REACTANT + i, "").length() != 0)
				found = true;
		if (!found) {
			showErrorMessage("There are no structures assigned to any of the "+reactantCount+" generic reactants.");
			return false;
			}
	...
	public void runTask(Properties configuration) {
		...
		Reactor theReactor = new Reactor(reaction, true);
		int dimensions = reaction.getReactants();
		for (int i=0; i<dimensions; i++)
			theReactor.setReactant(i, reactant[i][0]);
		...
		while (row < rowCount) {
			...
			while (currentDimension >= 0
				...
				theReactor.setReactant(currentDimension, reactant[currentDimension][0]);
				currentDimension--;
			}
			...
			theReactor.setReactant(currentDimension, reactant[currentDimension][index[currentDimension]]);
