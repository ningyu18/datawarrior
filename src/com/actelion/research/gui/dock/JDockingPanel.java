...
//	private ArrayList<ActionListener> mActionListenerList;
	...
	public JDockingPanel() {
		mDockableMap = new TreeMap<String,Dockable>();
		mLeafMap = new TreeMap<String,TreeLeaf>();
		mTargetPosition = -1;
	...
	public void eventDispatched(AWTEvent event) {
		...
		if (event.getID() == MouseEvent.MOUSE_RELEASED) {
			if (mTargetPosition == -1) {
				...
				if (targetTitle != null)
					relocateView(draggedTitle, targetTitle, mTargetPosition);
				}
			mTargetPosition = -1;
...
/*	private void performDrag() {
	...
	public void relocateView(String movedDockableName, String targetDockableName, int targetPosition) {
		...
		dock(movedDockable, targetPosition, mLeafMap.get(targetDockableName), 0.5, true);
		}
