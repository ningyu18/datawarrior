...
public class StructureCalculator {
	 ...
	 * Memory: O(nAtoms)
	 */
	public static List<List<Integer>> getConnexComponents(FFMolecule mol) {
	...
	public static int[] getAtomToGroups(FFMolecule mol) {
	 ...
	 */
	public static int[] getAtomToGroups(FFMolecule mol, List<Integer> seeds) {
	...
	public static List<Integer> getLongestChain(FFMolecule mol, int atm) {
	 ...
	 */
	public static boolean[] getBackbones(FFMolecule molecule, int minChain) {
	 ...
	 * 						      or -1  otherwise  
	 */
	public static int[][] getNumberOfBondsBetweenAtoms(FFMolecule mol, int maxBonds, int[][] dist) {
	 ...
	 * 						      or -1  otherwise  
	 */
	public static int[][] getNumberOfAtomsBetweenBonds(FFMolecule mol, int maxAtoms, int[][] dist) {
		//Initialization of the data
	...
	public static void main(String[] args) throws Exception {
		...
		FFMolecule m = new FFMolecule(sm);
	 ...
	 * @return
	 */
	public static int getImplicitHydrogens(FFMolecule mol, int atm) {
	...
	public static int getMaxFreeValence(FFMolecule mol, int atm) {
	 ...
	 * @return
	 */
	public static List<FFMolecule> extractLigands(FFMolecule mol) {
		List<List<Integer>> comps = getConnexComponents(mol);
		List<FFMolecule> res = new ArrayList<FFMolecule>();
		for (List<Integer> comp : comps) {
			...
			extractFragment(mol, m, ArrayUtils.toIntArray(comp));
			if(m.getAllAtoms()>5 && m.isAtomFlag(0, FFMolecule.LIGAND)) res.add(m);
		}
	 ...
	 * @return
	 */
	public static <T extends FFMolecule> T extractFragment(FFMolecule mol, T res, int[] atoms) {
		res.clear();
	...
	public static List<Integer>[] getRings(FFMolecule mol, List<int[]> allRings) { 
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static List<Integer>[] getRingsAccurate(FFMolecule mol, List<int[]> allRings) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static List<Integer>[] getRingsFast(FFMolecule mol, List<int[]> allRings) {
	...
	private static void doRings(FFMolecule mol, List<int[]> allRings, List<Integer> previous, boolean[] visitedBonds, int depth) {
		if(depth<=0) return;
	 ...
	 * @return
	 */
	public static List<int[]> getInterMolecularInteractions(FFMolecule mol) {
		//long s = System.currentTimeMillis();
	 ...
	 * @return
	 */
	public static int[] getAtomsOnPath(FFMolecule mol, int a1, int a2) {
	...
	public static int getValence(FFMolecule mol, int atm) {
		int res = 0;
		for(int i=0; i<mol.getAllConnAtoms(atm); i++) {
	...
	public static final int getExplicitHydrogens(FFMolecule mol, int atm) {
		int n = 0;
		for(int i=0; i<mol.getAllConnAtoms(atm); i++) {
	...
	public static int getStructureCenter(FFMolecule lig, int[] rotatables, int[][] dists) {
	 ...
	 * @return
	 */	
	public static int getStructureCenter(FFMolecule lig, int a, int[] rotatables, int[][] dists) {
	...
	public static List<Integer> getBackbone(FFMolecule lig, int a) {
	 ...
	 * @return
	 */
	public static FFMolecule extractLigand(FFMolecule mol) {
		if(mol==null) throw new IllegalArgumentException("The mol is null");
		if(mol.getNMovables()==mol.getAllAtoms()) return new FFMolecule(mol);
		FFMolecule res = new FFMolecule(mol.getName());
		int[] oldToNew = new int[mol.getAllAtoms()];
		Arrays.fill(oldToNew, -1);
		
		for(int i=0; i<mol.getAllAtoms(); i++) {
			if(mol.isAtomFlag(i, FFMolecule.LIGAND) && oldToNew[i]<0) {
...
//		res.setName((mol.getName()==null?"": mol.getName()) + (res.getAllAtoms()<mol.getAllAtoms()?" Ligand":""));
	...
	public static void insertLigand(FFMolecule mol, FFMolecule lig, Coordinates center) {
		//Add the new ligand
  		int index = mol.fusion(lig);
  		for(int i=index; i<mol.getAllAtoms(); i++) {
	  		mol.setAtomFlag(i, FFMolecule.LIGAND | FFMolecule.PREOPTIMIZED, true);
  		}
	 ...
	 * @param lig
	 */
	public static void replaceLigand(FFMolecule mol, FFMolecule lig, Coordinates center) {
	...
	public static boolean deleteHydrogens(FFMolecule mol) {
	 ...
	 * @return
	 */
	public static boolean addHydrogens(FFMolecule mol) {
	 ...
	 * @return
	 */
	public static boolean addHydrogens(FFMolecule mol, boolean alsoRigidAtoms) {
		boolean changed = false;
		int N = mol.getAllAtoms();
		...
		for(int i=0; i<N; i++) {
			if(!mol.isAtomFlag(i, FFMolecule.RIGID)) {
				//Ok, add H
			...
			if(n<0) {
				...
				for(int j=0; j<n; j++) {
					int a = mol.addAtom(1);
					mol.setAtomFlags(a, mol.getAtomFlags(i) & ~FFMolecule.PREOPTIMIZED);
					mol.addBond(i, a, 1);
	...
	public static boolean addHydrogensAroundLigand(FFMolecule mol, double radius) {
		boolean changed = false;
		int N = mol.getAllAtoms();
		MoleculeGrid grid = new MoleculeGrid(mol);
		List<Integer> atomsToBeDeleted = new ArrayList<Integer>();
		for(int i=0; i<N; i++) {
			if(mol.isAtomFlag(i, FFMolecule.RIGID)) {
				int neighbours = grid.getNeighbours(mol.getCoordinates(i), radius, true, FFMolecule.LIGAND).size();
	...
	public static void copyAtoms(FFMolecule mol, FFMolecule res, List<Integer> atomsToBeAdded) {
	...
	public static void removeWater(FFMolecule mol) {
		removeWater(mol, true);
	}

	public static void removeWater(FFMolecule mol, boolean removeAlsoImportant) {	
		if(mol==null) return;	
		for(int i = 0; i<mol.getAllAtoms(); i++) {
		...
		for(int i = 0; i<mol.getAllAtoms(); i++) {
			if(mol.getAtomicNo(i)==8 && (removeAlsoImportant || !mol.isAtomFlag(i, FFMolecule.IMPORTANT))) {
	...
	public static void removeWaterSalts(FFMolecule mol) {
		...
		for (int i = 0; i < groups.size(); i++) {
			...
			if(group.size()>20 || group.size()>mol.getAtoms()/30 || mol.isAtomFlag(group.get(0), FFMolecule.LIGAND)) continue;
			toRemove.addAll(groups.get(i));
		}
		mol.deleteAtoms(toRemove);
	}
	
	public static void removeLigand(FFMolecule mol) {
	...
	public static FFMolecule cropLigand(FFMolecule mol, double radius) {
		FFMolecule lig = extractLigand(mol);
		if(lig.getAllAtoms()==0) return new FFMolecule(mol);
	 ...
	 * @param n >=0
	 */
	public static int markLigand(FFMolecule mol, int n) {		
		...
		for(int i=0; i<l.size(); i++) {
			...
			for (int a :group) mol.setAtomFlag(a, FFMolecule.LIGAND, true);
			int S = SurfaceCalculator.getLigandSurface(mol);
			int SAS = SurfaceCalculator.getLigandSAS(mol);
			double buried = (1.0*S-SAS)/S;
			for (int a :group) mol.setAtomFlag(a, FFMolecule.LIGAND, false);
	 ...
	 */
	public static void markLigands(FFMolecule mol) {
		List<List<Integer>> l = StructureCalculator.getConnexComponents(mol);
		...
		for(int i=0; i<l.size(); i++) {
			...
			for(int j=0; j<group.size(); j++) {
				...
				if(potentials.contains(i)) {
					mol.setAtomFlag(atom, FFMolecule.LIGAND, true);						
					mol.setAtomFlag(atom, FFMolecule.RIGID, false);											
				} else {
					mol.setAtomFlag(atom, FFMolecule.RIGID, true);						
					mol.setAtomFlag(atom, FFMolecule.LIGAND, false);						
				}
			}
		}
	}
	 ...
	 * @param mol
	 */
	public static boolean markLigand(FFMolecule mol) {
		List<List<Integer>> l = StructureCalculator.getConnexComponents(mol);
		int bestGroup = -1;
		...
		if(bestGroup<0) {
			for (int i = 0; i < mol.getAllAtoms(); i++) {
				...
				mol.setAtomFlag(i, FFMolecule.RIGID, true);
			}
			for(int i=0; i<l.size(); i++) {
				List<Integer> group = l.get(i);
	...
	public static Coordinates getLigandCenter(FFMolecule mol) {
		...
		int n = 0;
		for(int i = 0; i<mol.getAllAtoms(); i++) {
			if(mol.getAtomicNo(i)>1 && mol.isAtomFlag(i, FFMolecule.LIGAND)) {
				sum = sum.addC(mol.getCoordinates(i));
				n++;
			}
		}				
	...
	public static Coordinates[] getLigandBounds(FFMolecule mol) {
		return getBounds(mol, FFMolecule.LIGAND);
	}

	public static Coordinates[] getBounds(FFMolecule mol) {
		return getBounds(mol, 0);
	}
	private static Coordinates[] getBounds(FFMolecule mol, int flags) {
	 ...
	 * @return
	 */
	public static double getLigandRMSD(FFMolecule mol1, FFMolecule mol2) {
		double sumSq = 0;
		int n = 0;
		for(int i1=0; i1<mol1.getAllAtoms(); i1++) {
			if(!mol1.isAtomFlag(i1, FFMolecule.LIGAND) || mol1.getAtomicNo(i1)<=1) continue;
			long s1 = getAtomHashkey(mol1, i1);
			double bestDistSq = Double.MAX_VALUE;
			int bestAtom = -1;
			for(int i2=0; i2<mol2.getAllAtoms(); i2++) {
				if(!mol2.isAtomFlag(i2, FFMolecule.LIGAND) || mol2.getAtomicNo(i2)!=mol1.getAtomicNo(i1)) continue;
	...
	public static void translateLigand(FFMolecule mol, Coordinates v) {
		for(int i = 0; i<mol.getAllAtoms(); i++) {
			if(mol.isAtomFlag(i, FFMolecule.LIGAND)) {
	 ...
	 */
	public static void vibrateLigand(FFMolecule mol, double radius) {
	 ...
	 * @return
	 */
	public static long getAtomHashkey(FFMolecule mol, int a) {
		long res = 0;
		for(int i=0; i<mol.getAllConnAtoms(a); i++) {
	...
	public static Coordinates[] getLigandCoordinates(FFMolecule mol) {		
		Coordinates[] coords = new Coordinates[mol.getNMovables()];
		System.arraycopy(mol.getCoordinates(), 0, coords, 0, coords.length);
		return coords;
	}
	
	public static void setLigandCoordinates(FFMolecule mol, Coordinates[] coords) {
		setLigandCoordinates(mol, coords, true);
	}
	
	public static void setLigandCoordinates(FFMolecule mol, Coordinates[] coords, boolean copyReferences) {
	...
	public static int makeProteinFlexible(FFMolecule mol, Coordinates center, double radius, boolean keepBackboneRigid) {
		...
		if(center==null) {
			MoleculeGrid grid = new MoleculeGrid(mol);	
			for (int i = 0; i < mol.getAllAtoms(); i++) {
				if(!mol.isAtomFlag(i, FFMolecule.LIGAND)) continue;			
				neighbours.addAll(grid.getNeighbours(mol.getCoordinates(i), radius));
			}
			
		} else {
			for (int i = 0; i < mol.getAllAtoms(); i++) {
				if(!mol.isAtomFlag(i, FFMolecule.LIGAND) && mol.getCoordinates(i).distanceSquared(center)<radius*radius) neighbours.add(i);
			}
		}
	...
	public static void makeProteinRigid(FFMolecule mol) {			
	...
	public static boolean isDonor(FFMolecule m, int a) {
	 ...
	 * @return
	 */
	public static List<int[]> getHBonds(FFMolecule m) {		
		...
		for (int i = 0; i < m.getNMovables(); i++) {
			if(!m.isAtomFlag(i, FFMolecule.LIGAND)) continue;
			...
			for (Iterator<Integer> iter = neighbours.iterator(); iter.hasNext();) {
				...
				if(m.isAtomFlag(a, FFMolecule.LIGAND)) continue;
	...
	public static final void createArtificialLigand(FFMolecule protein, Coordinates c, double radius) {
		...
		while(!l.isEmpty()) {
			...
			if(!grid.hasNeighbours(cc, 2.1)) {
				...
				protein.setAtomFlags(a, FFMolecule.LIGAND);
	...
	/*
	public static void normalizeCoordinates(FFMolecule mol) {
	...
	public static void normalizeCoordinates(FFMolecule mol, int a1, int a2, int a3) {
	...
	*/
	...
	////////////////////////////////////////////////////////////////
	public static void flagBackbone(FFMolecule mol) {
		boolean[] backbone = getBackbones(mol);
		for (int i = 0; i < backbone.length; i++) {
			mol.setAtomFlag(i, FFMolecule.BACKBONE, backbone[i]);
		}
	}

	public static int[] getRotatableBonds(FFMolecule mol, boolean considerHydrogens) {
	 ...
	 * @return
	 */
	public static int[] getRotatableBonds(FFMolecule mol, int groupSeed, boolean considerHydrogens) {
	...
	public static int getNRotatableBonds(FFMolecule mol) {
		return getRotatableBonds(mol, false).length;
	}

	public static boolean fixParities(FFMolecule mol, StereoMolecule ref) {
		return fixOrCheckParities(mol, ref, true);
	}
	public static boolean checkParities(FFMolecule mol, StereoMolecule ref) {
	 ...
	 */
	private static boolean fixOrCheckParities(FFMolecule mol, StereoMolecule ref, boolean allowChange) {
		mol.reorderAtoms();
...
//			System.out.println("CIP does not match "+sm.getAtomCIPParity(i)+"<>"+ref.getAtomCIPParity(i));
	...
	public static final double[][] getDistanceMatrix(FFMolecule m1, FFMolecule m2) {
	...
	public static double getSimilarity3D(FFMolecule m1, FFMolecule m2, Coordinates center, double radius) {
	 ...
	 */
	protected static double getSimilarity3DIntransitive(FFMolecule m1, FFMolecule m2, MoleculeGrid grid2, Coordinates center, double radius) {
		double sim = 0;
	 ...
	 */
	public static final int[] getOverlap(FFMolecule m1, FFMolecule m2) {		
	...
	public static String testLipinskiRules(StereoMolecule m, int weight, int donors, int acceptors, int rot, double clogpmin, double clogpmax, double clogsmin, double clogsmax) {		
		...
		int rota = getNRotatableBonds(new FFMolecule(m));
	 ...
	 * @return
	 */
	public static int getMolecularPathCount(FFMolecule mol, int length) {
	 ...
	 * @return
	 */
	public static List<int[]> getFragmentMatches(FFMolecule fragment, FFMolecule mol) {
		List<int[]> res = new ArrayList<int[]>();		
		getFragmentMatchesRec(fragment, mol, new int[fragment.getAllAtoms()], 0, res);
		return res;
	}
	
	private static void getFragmentMatchesRec(FFMolecule fragment, FFMolecule mol, int[] match, int fragmentMatchIndex, List<int[]> res) {
